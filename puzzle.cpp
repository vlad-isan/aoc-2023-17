...
uint64_t dijkstra(const Grid &grid) {
    ...
    std::vector<std::vector<int64_t>> dirs = {
        {0, 1},
        {0, -1},
        {1, 0},
        {-1, 0}
    ...
    while (!pq.empty()) {
        ...
        for (auto &dir: next_dirs) {
            ...
            if (x >= 0 && x < n && y >= 0 && y < m) {
                pq.push({
                    .position = {x, y}, .distance = node.distance + grid[x][y], .dir = dir,
                    .dir_count = node.dir == dir ? node.dir_count + 1 : 1
                });
            }
        }
    }
...
uint64_t dijkstra_2(const Grid &grid) {
    ...
    while (!pq.empty()) {
        ...
        bool is_starting_node = i == 0 && j == 0;

        if (!is_starting_node && node.dir_count < 10) {
            next_dirs.push_back(node.dir);
        }

        if (is_starting_node || node.dir_count >= 4) {
            for (auto &dir: dirs) {
                if (node.dir[0] == -dir[0] && node.dir[1] == -dir[1]) {
                    continue;
                }
        ...
        for (auto &dir: next_dirs) {
            ...
            // Don't allow to move in the same direction more than 3 times
            if (node.dir == dir && node.dir_count == 10) {
                continue;
            }
